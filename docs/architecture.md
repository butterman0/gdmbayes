...
- **`spGDMM`** (`models/_spgdmm.py`): Pure Bayesian estimator. Delegates preprocessing to `self.preprocessor` (a `GDMPreprocessor`). `fit()` calls `preprocessor.fit()`, builds a PyMC model, runs MCMC, and returns `InferenceData`. Prediction uses `preprocessor.transform()` directly.
