...
- **nutpie initvals**: nutpie 0.16.x ignores both `initvals` and `init_mean` kwargs. `_sample_nutpie()` works around this by compiling the model directly and replacing `initial_point_func` via `dataclasses.replace` on the compiled model. This is the only working mechanism for custom initial values with nutpie.
